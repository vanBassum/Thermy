...
void SettingsMenuPage::OnCreate()
{
    ...
    static constexpr lv_coord_t btnW = 140, btnH = 100, gap = 15;
    ...
    auto makeTile = [&](const char *text, lv_color_t color, lv_coord_t x, const char *target) {
        lv_obj_t *btn = AddButton(text, color, btnW, btnH, navTo);
        lv_obj_set_pos(btn, x, y);
        lv_obj_set_user_data(btn, const_cast<char *>(target));
    };

    makeTile(LV_SYMBOL_WIFI "\nWiFi", lv_palette_main(LV_PALETTE_BLUE), startX, "wifi");
    makeTile(LV_SYMBOL_EYE_OPEN "\nSensors", lv_palette_main(LV_PALETTE_GREEN), startX + btnW + gap, "sensors");
    makeTile(LV_SYMBOL_SETTINGS "\nSystem", lv_palette_main(LV_PALETTE_GREY), startX + 2 * (btnW + gap), "system");
